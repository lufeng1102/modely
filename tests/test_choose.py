...
def test_choose_resource_ranks_candidates(monkeypatch):
    monkeypatch.setattr("modely.choose.resolve_resource", lambda *a, **k: FakeResolve())
    monkeypatch.setattr("modely.choose.score_resource", lambda uri, **k: AssetScore(uri, 80 if uri.startswith("hf") else 95, "A", ScoreBreakdown()))
    monkeypatch.setattr("modely.choose.scan_resource", lambda uri, **k: ScanResult(uri, "low", analysis=AssetAnalysis(RepoInfo("hf", "model", "org/a"), FileSummary())))
